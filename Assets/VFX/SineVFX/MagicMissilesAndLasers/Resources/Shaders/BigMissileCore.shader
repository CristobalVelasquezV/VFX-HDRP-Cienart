...
////// Emissive:
                ...
                float2 node_743 = (i.uv0+node_6328.g*float2(0,0.75)).rg;
                float4 _Distortion_var = tex2D(_Distortion,TRANSFORM_TEX(i.uv0, _Distortion));
                float _DistortionAmount_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DistortionAmount );
                float node_7574 = (_Distortion_var.r*_DistortionAmount_var);
                float2 node_6573 = float2(node_743.r,(node_743.g+node_7574));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_6573, _Noise));
                float2 node_8814 = (i.uv0+node_6328.g*float2(0,0.5)).rg;
                ...
                float node_5564 = (1.0 - pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelExp_var));
                float3 emissive = (_Ramp_var.rgb*_TintColor_var.rgb*_FinalPower_var*node_2567.g*node_5564);
