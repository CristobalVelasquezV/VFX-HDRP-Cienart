    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                UNITY_DEFINE_INSTANCED_PROP( float, _FinalPower)
                UNITY_DEFINE_INSTANCED_PROP( float, _PrePower)
...
////// Emissive:
                ...
                float2 node_7138 = (i.uv0+node_7291.g*float2(0,0.05));
                ...
                float _PrePower_var = UNITY_ACCESS_INSTANCED_PROP( Props, _PrePower );
                float4 _Gradient2_var = tex2D(_Gradient2,TRANSFORM_TEX(i.uv0, _Gradient2));
                float node_9384 = (saturate((_FireTexture_var.r*_Mask_var.r*_PrePower_var))*_Gradient2_var.r*i.vertexColor.r*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection)))));
                float2 node_3126 = float2(node_9384,0.0);
                float4 _Ramp_var = tex2D(_Ramp,TRANSFORM_TEX(node_3126, _Ramp));
                float _FinalPower_var = UNITY_ACCESS_INSTANCED_PROP( Props, _FinalPower );
                float4 _Color_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Color );
                float3 emissive = (_Ramp_var.rgb*_FinalPower_var*_Color_var.rgb);
