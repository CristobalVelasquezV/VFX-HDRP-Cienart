...
Shader "Misc/Props HDRP"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_Brick_A("Brick_A", 2D) = "white" {}
		_Brick_N("Brick_N", 2D) = "bump" {}
		[ASEEnd]_Brick_R("Brick_R", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 uv_Brick_A = packedInput.ase_texcoord5.xy * _Brick_A_ST.xy + _Brick_A_ST.zw;
				
				float2 uv_Brick_N = packedInput.ase_texcoord5.xy * _Brick_N_ST.xy + _Brick_N_ST.zw;
				
				float2 uv_Brick_R = packedInput.ase_texcoord5.xy * _Brick_R_ST.xy + _Brick_R_ST.zw;
				
				surfaceDescription.Albedo = tex2D( _Brick_A, uv_Brick_A ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_Brick_A = packedInput.ase_texcoord.xy * _Brick_A_ST.xy + _Brick_A_ST.zw;
				
				float2 uv_Brick_N = packedInput.ase_texcoord.xy * _Brick_N_ST.xy + _Brick_N_ST.zw;
				
				float2 uv_Brick_R = packedInput.ase_texcoord.xy * _Brick_R_ST.xy + _Brick_R_ST.zw;
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_Brick_A = packedInput.ase_texcoord7.xy * _Brick_A_ST.xy + _Brick_A_ST.zw;
				
				float2 uv_Brick_N = packedInput.ase_texcoord7.xy * _Brick_N_ST.xy + _Brick_N_ST.zw;
				
				float2 uv_Brick_R = packedInput.ase_texcoord7.xy * _Brick_R_ST.xy + _Brick_R_ST.zw;
